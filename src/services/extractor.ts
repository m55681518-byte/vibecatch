...
import { resolveViaLocalNode, buildLocalStreamUrl, buildRelayStreamUrl, resolveViaRelayPool } from './localNode';
...
async function extractYouTube3Tier(cleanUrl: string): Promise<ExtractionResult> {
  ...
  // No local node running? Try the REMOTE relay POOL (workers.json) — failover
  // across healthy relays, zero-setup phone path (no Termux/Node install).
  const relayHit = await resolveViaRelayPool(videoId);
  if (relayHit && relayHit.audioUrl && relayHit.baseUrl) {
    ...
    const track: Track = {
      ...
      duration,
      thumbnailUrl: maxResThumb || thumbnailUrl,
      streamUrl: buildRelayStreamUrl(relayHit.baseUrl, relayHit.audioUrl),
      downloadUrl: buildRelayStreamUrl(relayHit.baseUrl, relayHit.audioUrl),
      platform: 'youtube',
      originalUrl: cleanUrl,
      addedAt: Date.now(),
      playsCount: 850000,
