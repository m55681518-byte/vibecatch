...
export function resolveCookiesPath(opts = {}) {
  ...
  // 3. State dir (bootstrap location; default %TEMP%/vibecatch-ytdlp)
  const baseDir = opts.stateDir || path.join(os.tmpdir(), 'vibecatch-ytdlp');
...
// Explicit config (opts.cookiesPath / env) always wins: never bootstrap then.
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    if (!fs.existsSync(p)) {
      const stub = [
        ...
        '# Auto-generated by vibecatch local node (bootstrap stub).',
...
export function startServer(port, opts = {}) {
  ...
  const stateDir = opts.stateDir || path.join(os.tmpdir(), 'vibecatch-ytdlp');
